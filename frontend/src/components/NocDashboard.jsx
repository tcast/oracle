...
/** Equirectangular with cos(midLat) horizontal scale so degrees aren't N-S stretched */
const US_MID_LAT = (US_LAT0 + US_LAT1) / 2;
const US_COS_MID = Math.cos((US_MID_LAT * Math.PI) / 180);
const US_MAP_PAD = 10;
const US_MAP_INNER_W = 590;
const US_MAP_INNER_H = Math.round(
  US_MAP_INNER_W * (US_LAT_SPAN / (US_LON_SPAN * US_COS_MID)),
);
const US_MAP_W = US_MAP_INNER_W + US_MAP_PAD * 2;
const US_MAP_H = US_MAP_INNER_H + US_MAP_PAD * 2;

/** Project lon/lat into SVG coords (same transform for outline and dots). No clamping. */
...
/** GeoJSON MultiPolygon → SVG path using projectUS (Natural Earth 50m CONUS). */
const usOutlinePath = (w, h) => {
  const feature = usConusGeo?.features?.[0];
