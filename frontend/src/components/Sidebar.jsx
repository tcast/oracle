...
const navItems = [
  { path: '/', label: 'Whisper', icon: 'M11 3.055A9.001 9.001 0 1020.945 13H11V3.055z M20.488 9H15V3.512A9.025 9.025 0 0120.488 9z' },
  { path: '/campaigns/new', label: 'AI Builder', icon: 'M8 12h.01M12 12h.01M16 12h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
  { path: '/brand-campaigns', label: 'Brand Campaigns', icon: 'M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z' },
  { path: '/brands', label: 'Companies', icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4' },
  { path: '/ads', label: 'Ad Builder', icon: 'M7 4v16l13-8L7 4zm-4 4v8m18-8v8' },
  ...
  { path: '/noc', label: 'NOC', icon: 'M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z' },
  { path: '/brain', label: 'Brain', icon: 'M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z' },
  { path: '/organic-activity', label: 'Organic Activity', icon: 'M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z' },
...
const Sidebar = ({ mobileOpen, onMobileClose }) => {
  ...
  const isActive = (path) => {
    ...
    if (path === '/brand-campaigns') {
      return location.pathname === '/brand-campaigns' || location.pathname.startsWith('/brand-campaigns/');
    }
    if (path === '/brands') return location.pathname.startsWith('/brands');
    ...
    <aside className={`${collapsed ? 'w-16' : 'w-60'} h-full bg-gray-900 flex flex-col transition-all duration-200 ease-in-out flex-shrink-0`}>
      <div className="flex items-center h-14 lg:h-16 px-4 border-b border-gray-800">
        <Link to="/" onClick={onMobileClose} className="flex items-center space-x-3 min-w-0">
          <div className="w-8 h-8 bg-gradient-to-br from-whisper-400 to-purple-600 rounded-lg flex items-center justify-center flex-shrink-0">
            <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
            </svg>
          </div>
          {!collapsed && <span className="text-lg font-bold text-white tracking-tight">Whisper</span>}
        </Link>
        ...
        {navItems.map(item => (
          <Link
            ...
            className={`flex items-center ${collapsed ? 'justify-center' : 'px-3'} py-2.5 rounded-lg text-sm font-medium transition-all duration-150 ${
              isActive(item.path)
                ? 'bg-whisper-600/20 text-whisper-400 shadow-sm'
      ...
      <div className="border-t border-gray-800 p-3">
        {collapsed ? (
          ...
          <div className="flex items-center space-x-3">
            <div className="w-8 h-8 bg-gradient-to-br from-whisper-500 to-purple-600 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
