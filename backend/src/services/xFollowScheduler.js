...
/**
 * X follow tick logic. Scheduling owned by durableQueue (BullMQ/Redis).
 */
class XFollowScheduler {
  constructor() {
    this.activeRuns = 0;
  }
  ...
  async tick() {
    const settings = await xFollowService.getSettings();
    if (!settings.enabled) {
    ...
    for (const account of accounts) {
      let job = await xFollowService.ensureJob(account.id);
      job = await xFollowService.refreshDayState(job, settings);
      ...
      if (xFollowService.inQuietHours(settings)) continue;
      due.push(account);
    ...
    await Promise.all(
      due.map(async (account) => {
        this.activeRuns += 1;
        try {
          const result = await xFollowService.runOneForAccount(account, { dryRun: false });
    ...
    if (results.length) {
      console.log(
        `X follow tick: ${results.length} job(s)`,
        results.map((r) => ({
...
module.exports = new XFollowScheduler();
